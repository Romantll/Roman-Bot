...
#Generating random kpop idol picture from file location "/idolpic"
...
async def idolpic(interaction: discord.Interaction, name: str):
    ...
    filepath = await get_idol_image(name)
    if filepath:
        file = discord.File(filepath)
        await interaction.followup.send (file=file, content=f"Here's a photo of **{name.title()}**!")
    else:
